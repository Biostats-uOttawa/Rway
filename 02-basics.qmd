...
You can also use `=` instead of `<-` to assign values but this is bad practice since it can lead to confusion later on when programming in `R` (see @sec-prog-r) and we would discourage you from using this notation.
...
Once you get a bit more experience with using R you may want to define your own functions to perform tasks that are specific to your goals (more about this in @sec-prog-r).
...
- [Internet search]: Use your favourite websearch engine (google, ecosia, duckduckgo, ... )for any error messages you get.
...
#### CRAN packages {#sec-cran-packages}
...
Perhaps the most efficient method is to use the `install_github()` function from the `remotes` :package: package (you installed this package previously (@sec-cran-packages)).
