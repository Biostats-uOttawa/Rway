...
To estimate the power of a paired t-test in R, we can use the function `pwr.t.test()`as for other t-tests but we need to specify the argument `type = "paired"`.
To calculate cohen's d for a paired t-test you need to use the formula $d = mean(\text{difference}) / sd(\text{difference})$ 
...
pwr.t.test(
  n = 15,
  d = mean(skull_w$diff) / sd(skull_w$diff),
  type = "paired")
```
