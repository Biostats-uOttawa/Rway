...
abline(v = -qc, col = "green")
lines(x1, y, type = "l", col = "blue", lwd = 2)

# power corresponds to the shaded area

polygon(
  c(x1[x1 <= -qc], -qc), c(y[x1 <= -qc], 0),
...
dat <- data.frame(
  ...
  mutate(
    area = ifelse(x1 < -qc, y, 0)
  )

ggplot(dat, aes(y = y)) +
  geom_line(aes(x = x), color = "red") +
  geom_line(aes(x = x1), color = "blue") +
  geom_vline(xintercept = c(-qc, qc), color = "green") +
  geom_area(
    aes(x = x1, y = area),
