...
`leaf area (mm^2)` or uppercase to simply your life).
...
The `fread()` function from the `data.table` :package: package is great for importing large data files quickly and efficiently (much faster than the `read.table()` function).
...
The `fwrite()` function from the `data.table` :package: package is very efficient at exporting large data objects and is much faster than the `write.table()` function.
